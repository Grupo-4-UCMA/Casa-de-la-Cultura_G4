...
BATCH_SIZE = 5000
LIMITE_RATINGS = 1000000000  # Para procesar absolutamente todo
        ...
        try:
            users.append(
                LibraryUser(
                    ...
                    comment=row.get("comentario") or row.get("preferences") or "",
                    birth_date=parse_date(row.get("fecha_nacimiento") or row.get("birthdate"))
                )
            )
        except Exception:
            continue
LibraryUser.objects.bulk_create(users, batch_size=BATCH_SIZE, ignore_conflicts=True)
...
Author.objects.bulk_create([Author(name=name) for name in author_names], batch_size=BATCH_SIZE, ignore_conflicts=True)

books_by_book_id = {book.book_id: book.id for book in Book.objects.only("id", "book_id")}
authors_by_name = {author.name: author.id for author in Author.objects.only("id", "name")}
...
for book_id, author_name in book_author_rows:
    ...
    if django_book_id and author_id:
        relations.append(through_model(book_id=django_book_id, author_id=author_id))

through_model.objects.bulk_create(relations, batch_size=BATCH_SIZE, ignore_conflicts=True)
...
Copy.objects.bulk_create(copies, batch_size=BATCH_SIZE, ignore_conflicts=True)
print(f"Copias cargadas: {Copy.objects.count()}")

print(f"Cargando ratings, límite actual: {LIMITE_RATINGS}...")
users_by_user_id = {user.user_id: user.id for user in LibraryUser.objects.only("id", "user_id")}
copies_by_copy_id = {copy.copy_id: copy.id for copy in Copy.objects.only("id", "copy_id")}
...
with open("data/ratings.csv", newline="", encoding="utf-8-sig") as f:
    ...
    for i, row in enumerate(reader, start=1):
        ...
        try:
            ...
            ratings.append(Rating(user_id=django_user_id, copy_id=django_copy_id, rating=rating_value))

            if len(ratings) >= BATCH_SIZE:
                Rating.objects.bulk_create(ratings, batch_size=BATCH_SIZE, ignore_conflicts=True)
                creados += len(ratings)
                ratings = []
                print(f"Ratings procesados: {i} | creados aprox: {creados} | omitidos: {omitidos}")
        except Exception:
            omitidos += 1
...
print("\n[PRE-PROCESADO] Generando archivo ultra-rápido de Votos para la web...")
import pandas as pd
try:
    ...
    print("[PRE-PROCESADO] ¡Listo! El catálogo cargará de forma instantánea.")
